...
\begin{itemize}
  \item DLO Dataset (\textcolor{red}{RAL}): Last week, I worked on real-time
  kernel and finally figured out how to configure and compile the source code.
  It was very complicated and complex, but I managed to document everything and
  it turns out to be a neat process.
  You can find the Real-Time Kernel Compilation and Installation document
  attached to this report.
  Moreover, I set up the new computer and it is ready for development as
  I would like it to be. Since it is fairly new, there is no Ubuntu driver
  support for the Intel WiFi card installed on the device. So I had to order a
  \$10 WiFi dongle for it. There is a way to install a generic driver for it but
  you still have to connect to the internet and download it every time a new
  OS installed. The ethernet interface did not work either, I used their adaptor
  dongle and did not recognize the connection for some reason. Today, I will
  continue RT-Kernel installation on the new computer (Alien) so I can verify it
  on the Panda arm tomorrow.
